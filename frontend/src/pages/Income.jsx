...
const Income = () => {
  ...
  const mockIncomes = [
    { id: 1, amount: 5000000, category: 'Uang Bulanan', destination: 'Saldo Bank', date: '2024-03-01', note: 'Dari Ortu' },
    { id: 2, amount: 500000, category: 'Freelance', destination: 'Cash', date: '2024-03-10', note: 'Project Design' },
  ];
  ...
  const fetchIncomes = async () => {
    try {
      ...
      console.warn("Using mock incomes");
  ...
  const handleAdd = async (e) => {
    ...
    try {
      await api.post('/transactions', { type: 'income', amount: parseFloat(amount), category, date, note: finalNote });
      setShowForm(false);
      fetchIncomes();
    } catch (err) {
      // simulate success for mock
      const newInc = { id: Date.now(), amount: parseFloat(amount), category, date, note: finalNote };
            ...
            <tbody>
              {incomes.length > 0 ? incomes.map((inc) => (
                <tr key={inc.id} className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30">
                  <td className="p-4 text-sm text-gray-900 dark:text-gray-100">{new Date(inc.date).toLocaleDateString()}</td>
                  ...
                  <td className="p-4 text-sm text-gray-500 dark:text-gray-400">{inc.note || '-'}</td>
                  <td className="p-4 text-sm font-semibold text-secondary text-right">+{formatCurrency(inc.amount)}</td>
                  <td className="p-4 flex justify-center gap-2">
                    <button onClick={() => handleDelete(inc.id)} className="text-gray-400 hover:text-danger p-1 transition-colors"><Trash2 size={16} /></button>
                  </td>
                </tr>
              )) : (
                <tr>
